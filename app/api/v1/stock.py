...
async def get_stock_list(
    ...
    Args:
        skip: 페이지네이션 오프셋
        limit: 페이지네이션 리미트
        market_code: 시장구분 필터 (0: KOSPI, 10: KOSDAQ, 30: K-OTC, 50: KONEX, 등)
...
async def sync_stock_list(
    market_code: Optional[MarketCode] = Query(MarketCode.KOSPI, description="시장구분 (0: KOSPI, 10: KOSDAQ, 등)"),
    stock_service: StockService = Depends(get_stock_service)
):
    """종목 리스트 동기화
    ...
    Args:
        market_code: 시장구분 (0: KOSPI, 10: KOSDAQ, 30: K-OTC, 50: KONEX, 등)

    Returns:
        동기화 결과 (저장된 종목 수)
    """
    try:
        result = await stock_service.sync_stock_list(market_code=market_code.value if market_code else "0")
        return APIResponse(
            success=True,
